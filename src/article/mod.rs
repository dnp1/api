...
pub fn register_handlers<'s>(db: Pool<PostgresConnectionManager>, router: &mut Router, sm: Arc<SessionManager>) {
    ...
    router.get("/article", SimpleHandlerBox::new(article_list, sm.clone()), "article_list");
    router.get("/article/:article_id", SimpleHandlerBox::new(article_read, sm.clone()), "article_read");
    router.get("/article/:article_id/content", SimpleHandlerBox::new(article_read_content, sm.clone()), "article_comment_create_content");
    router.get("/article/:article_id/tag", SessionHandlerBox { handler: article_tag_list, sm: sm.clone() }, "article_tag_list");
    router.get("/article/:article_id/comment", SimpleHandlerBox::new(article_comment_list, sm.clone()), "article_comment_list");
    router.get("/article/:article_id/comment/:comment_id", SimpleHandlerBox::new(article_comment_read, sm.clone() ), "article_comment_read");
    router.get("/article/:article_id/comment/:comment_id/content", SimpleHandlerBox::new(article_comment_read_content, sm.clone()), "article_comment_read_content");
